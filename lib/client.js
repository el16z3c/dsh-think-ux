...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* TEST BUILD (0.1.3-test.2): diagnostics on to verify the passive-clamp
		 * no-arm fix + the intent-TTL zombie fix in a real long run.
		...
		function apply(ctx) {
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					if (!st.follow || intentFresh(st)) continue;
