		...
		function apply(ctx) {
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					...
					if (diff < 0.5) { st.episode = null; continue; }
					 ...
					 * oscillation). */
					if (st.episode === null) st.episode = diff >= GAP_FAST_MIN ? "fast" : "smooth";
					else if (st.episode !== "fast" && diff >= GAP_FAST_MIN) st.episode = "fast";
			...
			function bindScroller(sc) {
				...
				bound.add(sc);
				const st = { top: sc.scrollTop, floor: floorOf(sc), intentSince: 0, readerRows: 0, jumpQueued: false, origOwn: undefined, listeners: null, rawSet: undefined, chaseWrite: null, episode: null, protoScrollTo: undefined, protoScrollBy: undefined, follow: (sc.scrollTop + AT_BOTTOM_EPS >= floorOf(sc)) };
