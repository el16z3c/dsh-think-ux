 ...
 *     auto-expanded, the body is a capped preview (at most 16 lines,
 *     tail-pinned so the newest text stays visible, top/bottom 24 px
 *     fades): the streaming "live preview" look instead of a full-height
 *     expansion. A reader toggle lifts the cap (their expansion is full).
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			 ...
			 * structural selector needs no hashed class names. The body's
			 * text tail is pinned to the bottom of the capped box (flex-end
			 * overflow clips the TOP, so the newest lines stay visible) and
			 * the mask fades both ends. */
			...
			function maybeExpand(row) {
				...
				requestAnimationFrame(() => {
					if (!mayOpen()) { managed.delete(row); capRow(row, false); return; }
			...
			function onScrollMove(sc, st) {
				...
				if (top > st.top + 0.5) {
					...
					if (floor - top <= RE_FLOOR_EPS) {
						disarm(st);
						if (floor - top > 0.5) sc.scrollTop = sc.scrollHeight;
					}
				} else if (top < st.top - 0.5) {
					arm(sc, st); /* upward drift, any device (scrollbar included) */
				}
				st.top = sc.scrollTop;
				st.floor = floor;
			}
			...
			function bindScroller(sc) {
				...
				bound.add(sc);
				const st = { top: sc.scrollTop, floor: floorOf(sc), intentSince: 0, readerRows: 0, jumpQueued: false, origOwn: undefined, listeners: null };
			...
			ctx.effect(() => {
				...
				return () => {
					...
					for (const row of managed) if (row.isConnected) capRow(row, false);
					capStyle.remove();
					managed.clear();
					userToggled.clear();
					rowState.clear();
