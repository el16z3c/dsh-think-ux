...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function trace(msg) {
			...
			console.debug(msg);
			if (TRACE_SINK_URL !== null) {
				try {
					fetch(TRACE_SINK_URL, { method: "POST",
		...
		function apply(ctx) {
			...
			function onScrollMove(sc, st, ev) {
				...
				if (DIAGNOSTICS && Math.abs(top - st.top) > 16 && !chaseEcho) {
					/* tr=1: browser-native scroll (user gesture, scroll
					 * anchoring, scrollIntoView, focus) — never JS writes.
					 * tr=0: a JS scrollTop write that bypassed the trap. */
			...
			function bindScroller(sc) {
				...
				bound.add(sc);
				const st = { id: ++scIndex, top: sc.scrollTop, floor: floorOf(sc), intentSince: 0, readerRows: 0, jumpQueued: false, origOwn: undefined, listeners: null, rawSet: undefined, chaseWrite: null, episode: null, episodeStart: 0, protoScrollTo: undefined, protoScrollBy: undefined, follow: (sc.scrollTop + AT_BOTTOM_EPS >= floorOf(sc)) };
			...
			ctx.effect(() => {
				const probe = DIAGNOSTICS ? installWriteProbe() : null;
