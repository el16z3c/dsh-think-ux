		...
		function apply(ctx) {
			...
			function followFrame(ts) {
				...
				for (const rec of follow.values()) {
					...
					body.scrollTop = Math.abs(diff) <= 2 ? target : body.scrollTop + diff * k;
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					...
					const next = diff <= 2 ? floor : top + diff * k;
			...
			function bindScroller(sc) {
				...
				bound.add(sc);
				const st = { top: sc.scrollTop, floor: floorOf(sc), intentSince: 0, readerRows: 0, jumpQueued: false, origOwn: undefined, listeners: null, rawSet: undefined, follow: (sc.scrollTop + AT_BOTTOM_EPS >= floorOf(sc)) };
