	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					...
					if (st.episode !== null && DIAGNOSTICS)
						trace("[think-ux] land ep=" + st.episode + " " +
					...
					if (st.episode === null) {
						...
						if (st.episode === "fast" && DIAGNOSTICS)
							trace("[think-ux] FAST episode gap=" + Math.round(diff) + "px");
					} else if (st.episode !== "fast" && diff >= GAP_FAST_MIN) {
						st.episode = "fast";
						if (DIAGNOSTICS) trace("[think-ux] fast upgrade gap=" + Math.round(diff) + "px");
					}
			...
			function arm(sc, st) {
				...
				if (DIAGNOSTICS) trace("[think-ux] intent armed (top=" +
			...
			function onScrollMove(sc, st) {
				...
				if (DIAGNOSTICS && Math.abs(top - st.top) > 16 && !chaseEcho) {
					trace("[think-ux] uncaught motion: top=" + Math.round(top) +
						...
						set: function (v) {
							if (st.intentSince !== 0 &&
								...
								if (top + AT_BOTTOM_EPS < floor && v - top > AT_BOTTOM_EPS &&
									...
									set.call(this, v);
									set.call(this, top);
									trace("[think-ux] re-pin reverted (reader at " +
							...
							if (DIAGNOSTICS) {
								...
								if (Math.abs(v - t0) > 16) {
									...
									trace("[think-ux] native write: top=" + Math.round(t0) +
