...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* Final build: off — no prototype probe, no console traces, no sink
		 * POSTs. To hunt a jank report again: true + TRACE_SINK_URL. */
		...
		/* Ship off; set to "http://127.0.0.1:3999/" + start trace-sink.cjs
		 * for a disk log (only meaningful with DIAGNOSTICS on). */
		...
		function apply(ctx) {
			...
			function maybeCollapse(row) {
				...
				if (COLLAPSE_MS > 0 && h > 2) {
					try {
						timer = setTimeout(finish, COLLAPSE_MS + 80);
						animated = true;
					} catch (e) { /* runner withheld timers: instant */ }
				}
				if (animated) {
					collapsing.set(row, { body: body, timer: timer });
					requestAnimationFrame(() => {
						const c = collapsing.get(row);
