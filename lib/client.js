...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* Chase velocity cap (px per 60 fps frame). The exponential chase has
		 * no speed limit, so a large gap (a new tool-call row or a body
		 * block landing in one commit, 100–400 px) swooshes at 20 %+ of the
		 * distance per frame — it reads as a snap. Capped: large gaps glide
		 * at a constant ~960 px/s, small gaps stay pure exponential (the
		 * handoff is continuous, no kink). */
		...
		function apply(ctx) {
			...
			function followFrame(ts) {
				...
				const cap = CHASE_MAX_PX * (dt / 16.667);
				let live = false;
				for (const rec of follow.values()) {
					...
					/* Same capped exponential as the main chase (the cap
					 * only bites on large re-flows). */
					const step = Math.abs(diff) <= 2
							? diff
							: Math.min(Math.abs(diff) * k, cap) * (diff > 0 ? 1 : -1);
			 ...
			 * exponential easing, so streamed output glides up instead of
			 * jumping in token chunks. Follow mode is on while the reader is
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					...
					/* Capped exponential: pure exponential on small gaps
					 * (streaming deltas), velocity-capped on large ones
					 * (a new tool-call row / body block) — continuous
					 * handoff at diff = cap/k, no kink. */
					const next = diff <= 2 ? floor : top + Math.min(diff * k, cap);
