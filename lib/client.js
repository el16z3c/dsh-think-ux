...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* Gap-dependent speed ("each gap closes at its own speed"):
		 * below GAP_RAMP_START the chase runs at the constant CHASE_MAX_PX
		 * speed (streaming inserts glide smoothly); above GAP_RAMP_END it
		 * is the uncapped exponential again (opening a long session
		 * swooshes to the bottom in ~1 s instead of crawling). Between
		 * the two the speed blends via smoothstep, so the regimes hand
		 * off with no kink. Rollback states: RAMP_END=0 -> pure
		 * exponential (pre-cap); RAMP_END=9999999 -> one flat speed. */
		const GAP_RAMP_START = 800;
		const GAP_RAMP_END = 2400;
		/* Diagnostics: console.debug traces of intent arming, uncaught large
		 * motion, and native (non-intercepted) scroll writes. On while
		 * hunting a jank report; flip off + redeploy to silence. Cheap:
		 * fires only on large motions/writes. */
		...
		function apply(ctx) {
			 ...
			 * px (positive). Gap-dependent speed:
			 *  - diff <  GAP_RAMP_START: constant CHASE_MAX_PX speed
			 *    (~960 px/s at 60 fps) — streaming inserts (a new tool
			 *    row / body block) glide smoothly, no swoosh;
			 *  - diff >= GAP_RAMP_END: the uncapped exponential
			 *    (21% of the gap per frame at 60 fps) — opening a long
			 *    session swooshes to the bottom (front-loaded decay),
			 *    not a multi-second crawl;
			 *  - between: smoothstep blend, flat at both ends -> C1
			 *    continuous, no visible kink.
			 *  - diff <= 2: the exact remainder (lands precisely). */
			function chaseStep(dt, diff) {
				if (diff <= 2) return diff;
				const k = 1 - Math.exp(-dt / CHASE_TAU_MS);
				const base = CHASE_MAX_PX * (dt / 16.667);
				if (diff < GAP_RAMP_START) return Math.min(diff * k, base);
				if (diff >= GAP_RAMP_END) return diff * k;
				const t = (diff - GAP_RAMP_START) / (GAP_RAMP_END - GAP_RAMP_START);
				const s = t * t * (3 - 2 * t); /* smoothstep: 0->1, zero slope at both ends */
				return base + (diff * k - base) * s;
			}
			...
			function followFrame(ts) {
				...
				for (const rec of follow.values()) {
					...
					/* Same gap-dependent step as the main chase. */
					const step = chaseStep(dt, Math.abs(diff)) * (diff > 0 ? 1 : -1);
			 ...
			 * the gap-dependent step (chaseStep: constant speed on
			 * streaming-insert gaps, fast exponential on session-open
			 * gaps), so streamed output glides up instead of jumping in
			 * token chunks. Follow mode is on while the reader is
			...
			function mainChaseFrame(ts) {
				...
				for (const [sc, st] of scrollerState) {
					...
					if (diff < 0.5) continue; /* tail reached */
					/* Gap-dependent step: small gaps (streaming inserts)
					 * close at the constant capped speed; large gaps
					 * (opening a session) close with the fast
					 * exponential; between them the speed blends
					 * smoothly — no kink, no crawl. */
					const next = top + chaseStep(dt, diff);
			...
			function onScrollMove(sc, st) {
				...
				st.chaseWrite = null;
				if (DIAGNOSTICS && Math.abs(top - st.top) > 40 && !chaseEcho) {
			...
			function bindScroller(sc) {
				...
				bound.add(sc);
				const st = { top: sc.scrollTop, floor: floorOf(sc), intentSince: 0, readerRows: 0, jumpQueued: false, origOwn: undefined, listeners: null, rawSet: undefined, chaseWrite: null, protoScrollTo: undefined, protoScrollBy: undefined, follow: (sc.scrollTop + AT_BOTTOM_EPS >= floorOf(sc)) };
